...
# 保护所有「load → modify → save」序列，防止并发请求（心跳 vs 命令等待）互相覆盖
...
def apply_address_change_to_data(data, source_address, target_address, slot_id=None, radar_name=None):
    """仅更新平台数据库中的雷达地址记录，不做任何串口/边缘写操作。

    当边缘设备已成功执行 change_address 后调用，用于同步云端记录。
    """
    serial_profile = get_default_serial_profile(data)
    modbus_profile = get_default_modbus_profile(data)
    # 查找源地址对应的雷达（edge 模式用 source_address 匹配；255 表示广播地址编程）
    radar = next((r for r in data["radars"] if r.get("address") == source_address), None)
    if radar is None:
        radar = normalize_radar({
    ...
    cleanup_radar_slot_links(data)
    push_operation_history(data, "modbus.address_program", {
        "sourceAddress": source_address,
        "targetAddress": target_address,
        "slotId": slot_id or "",
    })
    add_log("success", "雷达地址记录已更新：%s → %s" % (source_address, target_address))
    save_data(data)
...
def complete_edge_command(data, device_id, command_id, payload):
    ...
    prev_status = command.get("status")
    ...
    save_data(data)
    # 若 change_address 成功（含云端已超时但 ESP32 仍执行的情况），自动同步数据库
    if success and command.get("action") == "change_address":
        src = command.get("address")
        tgt = command.get("targetAddress")
        if src is not None and tgt is not None:
            try:
                fresh = load_data()  # 重新加载，保证基于最新数据修改
                apply_address_change_to_data(fresh, src, tgt)
                if prev_status == "failed":
                    add_log("success", "[边缘] change_address 延迟完成：%s → %s（云端已超时，数据库已补更新）" % (src, tgt))
                else:
                    add_log("success", "[边缘] change_address 完成，数据库已同步：%s → %s" % (src, tgt))
            except Exception as exc:
                add_log("warning", "[边缘] change_address 数据库同步失败：%s" % str(exc))
    return command
...
def execute_via_edge(device_id, action, command_payload, timeout=22, poll_interval=1.5):
    ...
    with DATA_LOCK:
        ...
        save_json(DATA_FILE, normalize_data(data))  # 直接写，避免 save_data 重入 DATA_LOCK
    logger.info("edge bridge: dispatched %s cmd=%s device=%s", action, command_id, device_id)

    _none_miss_count = 0  # 允许短暂的 cmd is None（并发写窗口）后重试
    ...
    while time.time() < deadline:
        ...
        if cmd is None:
            _none_miss_count += 1
            if _none_miss_count >= 3:  # 连续 3 次找不到，判定真正丢失
                logger.error("edge bridge: command lost cmd=%s", command_id)
                return None, "边缘命令记录丢失（并发写冲突）"
            continue  # 否则等下一轮重试
        _none_miss_count = 0
    ...
    # 超时：若命令已下发给 ESP32（delivered），保留命令让其自然完成（complete_edge_command 会补更新数据库）
    # 若仍是 pending（ESP32 未取走），标记为 failed
    with DATA_LOCK:
        fresh = load_json(DATA_FILE, default_data())
        fresh = normalize_data(fresh)
        cmd = next((c for c in fresh.get("edgeCommands", []) if c["id"] == command_id), None)
        if cmd:
            if cmd["status"] == "pending":
                cmd["status"] = "failed"
                cmd["completedAt"] = now_str()
                cmd["message"] = "云端等待超时（%ds），ESP32 未取走命令" % timeout
                save_json(DATA_FILE, normalize_data(fresh))
            elif cmd["status"] == "delivered":
                # ESP32 已取走，仍在执行；保留命令，complete_edge_command 回调时会同步地址
                cmd["message"] = "云端等待超时（%ds），ESP32 仍在执行，结果将在回调时自动同步" % timeout
                save_json(DATA_FILE, normalize_data(fresh))
...
def program_address():
    ...
    if not server_modbus_enabled():
        ...
        if err:
            ...
            return jsonify({"success": False, "message": err}), 400
        # 地址编程成功后仅更新数据库记录（不走串口）
        data = load_data()  # 重新加载，避免过期快照
        try:
            radar, slot = apply_address_change_to_data(data, source_address, target_address, slot_id=slot_id, radar_name=name)
